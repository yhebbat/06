...
void send_all(int exept) {
    for (int i = 0; i <= max; i++)
        if (FD_ISSET(i, &wfds) && i != exept)//fd_isset check if this fd is ready to write on it
            send(i, bufWrite, strlen(bufWrite), 0);//send msg to all other fds
}
...
int main(int ac, char **av) {
    ...
    while (1) {
        wfds = rfds = fds;
        if (select(max + 1, &rfds, &wfds, NULL, NULL) < 0)//how much fds ready to read or write
            continue ;
        for (int s = 0; s <= max; s++) {
            if (FD_ISSET(s, &rfds) && s == sockfd) {//new connection
